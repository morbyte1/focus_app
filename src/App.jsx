...
const Card = ({ children, className = "", onClick }) => <div onClick={onClick} className={`bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-white/5 rounded-3xl shadow-sm dark:shadow-lg p-6 ${className}`}>{children}</div>;
...
const Modal = ({ isOpen, onClose, title, children }) => {
  ...
  return createPortal(<div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/50 dark:bg-black/90 backdrop-blur-md animate-fadeIn"><div className="absolute inset-0" onClick={onClose}></div><div className="bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-3xl w-full max-w-md p-6 relative shadow-2xl max-h-[90vh] overflow-y-auto custom-scrollbar z-10"><button onClick={onClose} className="absolute top-5 right-5 text-zinc-500 hover:text-black dark:hover:text-white transition-colors"><X size={20}/></button><h2 className="text-xl font-bold text-zinc-900 dark:text-white mb-6 pr-8">{title}</h2>{children}</div></div>, document.body);
};
...
const SettingsView = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0"><header className="mb-8"><h1 className="text-2xl font-bold text-zinc-900 dark:text-white mb-1">Configurações</h1><p className="text-zinc-500 dark:text-zinc-400">Gerencie seus dados e preferências.</p></header>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card><h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-4 flex items-center gap-2"><Settings size={20} className="text-[#1100ab]" /> Aparência</h3>
        ...
        <Card><h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-4 flex items-center gap-2"><HardDrive size={20} className="text-[#1100ab]" /> Dados do Sistema</h3><div className="space-y-4"><div className="bg-zinc-50 dark:bg-zinc-900/50 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800"><p className="text-sm text-zinc-600 dark:text-zinc-300 mb-3">Exporte seus dados para backup ou importe de outro dispositivo.</p><div className="flex gap-3"><button onClick={hExp} className="flex-1 py-3 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white rounded-xl text-sm flex items-center justify-center gap-2 transition-colors"><Download size={16} /> Exportar</button><label className="flex-1 py-3 bg-zinc-200 dark:bg-zinc-800 hover:bg-zinc-300 dark:hover:bg-zinc-700 text-zinc-900 dark:text-white rounded-xl text-sm flex items-center justify-center gap-2 cursor-pointer transition-colors"><Upload size={16} /> Importar<input type="file" className="hidden" onChange={hImp} /></label></div></div><div className="bg-red-500/5 p-4 rounded-2xl border border-red-500/10"><p className="text-sm text-red-400 mb-3 font-bold">Zona de Perigo</p><button onClick={resetAllData} className="w-full py-3 bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white rounded-xl text-sm flex items-center justify-center gap-2 transition-all"><Trash2 size={16} /> Resetar TUDO (Fábrica)</button></div></div></Card>
        <Card><h3 className="text-lg font-bold text-zinc-900 dark:text-white mb-4 flex items-center gap-2"><Crown size={20} className="text-yellow-500" /> Gamificação</h3><div className="space-y-4"><div className="bg-zinc-50 dark:bg-zinc-900/50 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800"><p className="text-sm text-zinc-600 dark:text-zinc-300 mb-3">Reiniciar apenas seu progresso de níveis e XP.</p><button onClick={resetXPOnly} className="w-full py-3 bg-yellow-500/10 hover:bg-yellow-500 text-yellow-500 hover:text-black rounded-xl text-sm flex items-center justify-center gap-2 transition-all"><RotateCcw size={16} /> Resetar Nível e XP</button></div></div></Card>
      </div>
    </div>
  );
};
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0">
      {/* CARTÃO DE NÍVEL / RANK:
         - Mudança: Classes 'bg-gradient-to-r' agora têm versões light e dark.
         - Light: from-[#1100ab] to-[#4d4dff] (Azul vibrante)
         - Dark: from-[#1100ab]/80 via-[#0a0a0a] to-[#0a0a0a] (Preto original)
      */}
      <Card className="bg-gradient-to-r from-[#1100ab] to-[#4d4dff] dark:from-[#1100ab]/80 dark:via-[#0a0a0a] dark:to-[#0a0a0a] border-[#1100ab]/30 overflow-hidden relative">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center relative z-10">
          ...
          <div className="flex-1 w-full"><div className="flex justify-between items-end mb-2"><div><h2 className="text-2xl font-bold text-white">{userLevel.title}</h2><p className="text-sm text-zinc-100 dark:text-zinc-400">Total: {userLevel.totalXP.toLocaleString()} XP</p></div><div className="hidden sm:block text-white dark:text-[#4d4dff] font-bold">{userLevel.currentXP} <span className="text-zinc-200 dark:text-zinc-500">/ {xpNext} XP</span></div></div><div className="w-full h-4 bg-black/20 dark:bg-zinc-800 rounded-full overflow-hidden border border-white/10 dark:border-zinc-700 relative"><div className={`h-full bg-gradient-to-r ${rank.b} transition-all duration-1000`} style={{ width: `${(userLevel.currentXP / xpNext) * 100}%` }} /><div className="absolute top-0 right-0 bottom-0 w-[1px] bg-white/50 shadow-[0_0_10px_white]" /></div><p className="text-xs text-zinc-200 dark:text-zinc-500 mt-2 text-right sm:hidden">{userLevel.currentXP} / {xpNext} XP</p></div>
        </div>
      </Card>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[{ l: 'Hoje', v: `${kpiData.todayMinutes} min`, i: Zap, c: 'yellow-500' }, { l: 'Total', v: `${kpiData.totalHours} h`, i: Clock, c: '[#1100ab]', ic: '[#4d4dff]' }, { l: 'Sequência', v: `${kpiData.streak} dias`, i: Flame, c: 'orange-500' }].map((x, i) => <Card key={i} className={`flex items-center gap-4 border-l-4 border-l-${x.c}`}><div className={`p-3 bg-${x.c}/20 rounded-full text-${x.ic || x.c}`}><x.i size={24} /></div><div><p className="text-sm text-zinc-500 dark:text-zinc-400">{x.l}</p><p className="text-2xl font-bold text-zinc-900 dark:text-white">{x.v}</p></div></Card>)}
        <Card onClick={() => { setForm(countdown); setModal(true); }} className={`flex items-center gap-4 border-l-4 cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800/80 transition-all group ${countStyle.c}`}><div className={`p-3 rounded-full transition-colors ${countStyle.b}`}><Flag size={24} /></div><div><p className="text-sm text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-900 dark:group-hover:text-white transition-colors">{daysLeft !== null ? (countdown.title || "Meta") : "Próxima Meta"}</p>{daysLeft !== null ? (<p className={`text-2xl font-bold ${daysLeft <= 7 ? 'scale-105 origin-left' : ''} transition-transform`}>{daysLeft === 0 ? "É HOJE!" : daysLeft < 0 ? "Concluído" : `${daysLeft} dias`}</p>) : <p className="text-sm font-bold text-zinc-400 dark:text-zinc-500 italic">Definir data...</p>}</div></Card>
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 min-h-[300px]"><h3 className="text-lg font-semibold text-zinc-900 dark:text-white mb-4 flex items-center gap-2"><BarChart2 size={20} className="text-[#1100ab]" /> Atividade Semanal</h3><div className="h-[250px] w-full"><ResponsiveContainer><LineChart data={weeklyChartData}><CartesianGrid strokeDasharray="3 3" stroke="#52525b" opacity={0.3} vertical={false} /><XAxis dataKey="name" stroke="#71717a" tick={{ fill: '#a1a1aa', fontSize: 12 }} axisLine={false} /><YAxis stroke="#71717a" tick={{ fill: '#a1a1aa' }} axisLine={false} /><Tooltip contentStyle={{ backgroundColor: '#09090b', borderColor: '#27272a', color: '#fff', borderRadius: '12px' }} /><Line type="monotone" dataKey="minutos" stroke="#1100ab" strokeWidth={3} dot={{ r: 4, fill: '#1100ab' }} /></LineChart></ResponsiveContainer></div></Card>
        
        {/* CARTÃO TRABALHO PROFUNDO:
           - Mudança: Classes 'bg-gradient-to-br' atualizadas.
           - Light: from-[#1100ab] to-[#4d4dff] (Azul vibrante)
           - Dark: from-[#1100ab]/40 to-[#0a0a0a] (Preto original)
        */}
        <Card className="bg-gradient-to-br from-[#1100ab] to-[#4d4dff] dark:from-[#1100ab]/40 dark:to-[#0a0a0a] border-[#1100ab]/30 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div><h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">Trabalho Profundo <InfinityIcon size={18} className="text-white dark:text-[#4d4dff]" /></h3><p className="text-zinc-100 dark:text-zinc-300 mb-6 italic text-sm border-l-2 border-white/50 dark:border-[#1100ab]/50 pl-3">"A superficialidade não constrói impérios."</p></div>
            <Button onClick={() => setCurrentView('focus')} className="w-full py-4 text-lg shadow-lg shadow-black/20 dark:shadow-[#1100ab]/20 hover:shadow-black/40 dark:hover:shadow-[#1100ab]/40 border border-white/20 dark:border-white/5 bg-white/20 hover:bg-white/30 dark:bg-[#1100ab] dark:hover:bg-[#0c007a] text-white">Mergulhar no Foco <Zap size={20} className="fill-current" /></Button>
        </Card>
      </div>
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Configurar Meta"><form onSubmit={e => { e.preventDefault(); setCountdown({ date: form.date, title: form.title || "Minha Meta" }); setModal(false); }} className="space-y-4"><div><label className="text-xs text-zinc-500 font-bold uppercase">Nome da Meta</label><input type="text" placeholder="Ex: ENEM..." className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-[#1100ab]" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} /></div><div><label className="text-xs text-zinc-500 font-bold uppercase">Data Alvo</label><input type="date" required className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-[#1100ab]" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} /></div><div className="pt-2 flex gap-2"><Button type="button" variant="secondary" onClick={() => { setCountdown({ date: null, title: '' }); setModal(false); }} className="flex-1">Limpar</Button><Button type="submit" className="flex-[2]">Salvar Data</Button></div></form></Modal>
    </div>
  );
};
...
const FocusView = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn pb-24 md:pb-0">
      ...
      <Modal isOpen={finMod} onClose={() => setFinMod(false)} title="Resumo da Sessão"><form onSubmit={finish} className="space-y-4"><div><label className="text-xs text-zinc-500 font-bold uppercase">O que você estudou?</label><textarea className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-sm text-zinc-900 dark:text-white h-24 outline-none focus:border-[#1100ab] resize-none" value={fForm.n} onChange={e => setFForm({ ...fForm, n: e.target.value })} /></div><div className="grid grid-cols-2 gap-4"><div><label className="text-xs text-zinc-500 font-bold uppercase">Questões Feitas</label><input type="number" min="0" className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-blue-500" value={fForm.q} onChange={e => setFForm({ ...fForm, q: e.target.value })} /></div><div><label className="text-xs text-zinc-500 font-bold uppercase">Erradas</label><input type="number" min="0" className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-red-500" value={fForm.e} onChange={e => setFForm({ ...fForm, e: e.target.value })} /></div></div><div className="pt-2 flex gap-2"><Button type="button" variant="secondary" onClick={() => setFinMod(false)} className="flex-1">Cancelar</Button><Button type="submit" className="flex-[2]">Salvar Sessão</Button></div></form></Modal>
      <Modal isOpen={manMod} onClose={() => setManMod(false)} title="Registro Manual"><form onSubmit={manual} className="space-y-5"><div className="space-y-2"><label className="text-xs font-bold text-zinc-500 uppercase">Matéria</label><select required className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-[#1100ab]" value={mForm.s} onChange={e => setMForm({ ...mForm, s: e.target.value })}>{subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div><div className="space-y-2"><label className="text-xs font-bold text-zinc-500 uppercase">Tempo (min)</label><input required type="number" min="1" className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-[#1100ab]" value={mForm.t} onChange={e => setMForm({ ...mForm, t: e.target.value })} /></div><div className="space-y-2"><label className="text-xs font-bold text-zinc-500 uppercase">Diário</label><textarea className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-2xl p-3 text-zinc-900 dark:text-white h-28 outline-none focus:border-[#1100ab] resize-none text-sm" value={mForm.n} onChange={e => setMForm({ ...mForm, n: e.target.value })} /></div><div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-zinc-500 uppercase">Feitas</label><input type="number" min="0" className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-blue-500" value={mForm.q} onChange={e => setMForm({ ...mForm, q: e.target.value })} /></div><div><label className="text-xs font-bold text-zinc-500 uppercase">Erradas</label><input type="number" min="0" className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-800 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-red-500" value={mForm.e} onChange={e => setMForm({ ...mForm, e: e.target.value })} /></div></div><Button type="submit" className="w-full py-3 mt-2 shadow-xl shadow-[#1100ab]/20">Confirmar</Button></form></Modal>
    </div>
  );
};
...
const MistakesView = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0"><header><h1 className="text-2xl font-bold text-zinc-900 dark:text-white mb-1">Caderno de Erros</h1></header>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="h-fit"><h3 className="text-zinc-900 dark:text-white font-semibold mb-4 flex items-center gap-2"><AlertTriangle size={18} className="text-red-500" /> Novo Erro</h3><form onSubmit={e => { e.preventDefault(); if (f.sub && f.desc && f.sol) { addMistake(f.sub, f.desc, f.r, f.sol); setF({ sub: "", desc: "", r: "Conceito", sol: "" }); alert("Salvo!"); } }} className="space-y-4"><div><label className="text-xs text-zinc-500 font-bold uppercase">Matéria</label><select required className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-2.5 text-zinc-900 dark:text-white" value={f.sub} onChange={e => setF({ ...f, sub: e.target.value })}><option value="">Selecione...</option>{subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div><div><label className="text-xs text-zinc-500 font-bold uppercase">Motivo</label><div className="flex flex-wrap gap-2 mt-2">{["Falta de Atenção", "Esqueci Fórmula", "Erro Cálculo", "Conceito", "Tempo"].map(r => <button key={r} type="button" onClick={() => setF({ ...f, r })} className={`text-xs px-3 py-1.5 rounded-full border ${f.r === r ? 'bg-[#1100ab] border-[#1100ab] text-white' : 'border-zinc-300 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400'}`}>{r}</button>)}</div></div><div><label className="text-xs text-zinc-500 font-bold uppercase">Erro</label><textarea required className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-sm text-zinc-900 dark:text-white h-20" value={f.desc} onChange={e => setF({ ...f, desc: e.target.value })} /></div><div><label className="text-xs text-zinc-500 font-bold uppercase">Solução</label><textarea required className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-sm text-zinc-900 dark:text-white h-24" value={f.sol} onChange={e => setF({ ...f, sol: e.target.value })} /></div><Button type="submit" className="w-full">Salvar</Button></form></Card>
        <div className="lg:col-span-2 space-y-4">{!mistakes.length ? <div className="text-center py-10 opacity-50"><CheckCircle size={40} className="mx-auto mb-2" /><p>Vazio.</p></div> : mistakes.map(m => <div key={m.id} className={`border rounded-3xl p-5 relative group transition-all duration-300 ${m.consolidated ? 'bg-zinc-100 dark:bg-zinc-900/50 border-zinc-200 dark:border-zinc-800 opacity-75' : 'bg-white dark:bg-[#09090b] border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700'}`}>{m.consolidated && <div className="absolute top-4 right-12 bg-green-500/10 text-green-500 text-[10px] font-bold px-2 py-1 rounded border border-green-500/20 flex items-center gap-1 select-none"><CheckCircle size={10} /> ERRO APRENDIDO</div>}<div className="absolute top-4 right-4 flex gap-2">{!m.consolidated && <button onClick={() => setCons({ open: true, id: m.id, d: m.description, diag: "", strat: "" })} className="text-zinc-400 hover:text-green-500 opacity-0 group-hover:opacity-100 transition-opacity"><CheckCircle size={18} /></button>}<button onClick={() => deleteMistake(m.id)} className="text-zinc-400 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={18} /></button></div><div className="flex items-center gap-3 mb-3"><span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase text-white" style={{ backgroundColor: subjects.find(s => s.id === m.subjectId)?.color }}>{subjects.find(s => s.id === m.subjectId)?.name}</span><span className="text-xs font-medium px-2 py-0.5 rounded-full bg-red-500/10 text-red-400 border border-red-500/20">{m.reason}</span></div><div className="grid md:grid-cols-2 gap-6"><div className="space-y-1"><p className="text-xs text-red-400 font-bold uppercase">Erro</p><MathRenderer className="text-zinc-600 dark:text-zinc-300 text-sm" text={m.description} /></div><div className="space-y-1 md:border-l md:border-zinc-200 dark:md:border-zinc-800 md:pl-6"><p className="text-xs text-green-400 font-bold uppercase">Solução</p><MathRenderer className="text-zinc-600 dark:text-zinc-300 text-sm" text={m.solution} /></div></div>{m.consolidated && <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800/50"><button onClick={() => setExp(p => p.includes(m.id) ? p.filter(x => x !== m.id) : [...p, m.id])} className="text-xs text-[#4d4dff] hover:text-zinc-900 dark:hover:text-white flex items-center gap-1 transition-colors">{exp.includes(m.id) ? 'Ocultar reflexão' : 'Ver minha reflexão'} <ChevronDown size={14} className={`transition-transform ${exp.includes(m.id) ? 'rotate-180' : ''}`} /></button>{exp.includes(m.id) && <div className="mt-3 grid md:grid-cols-2 gap-4 animate-fadeIn"><div className="bg-zinc-50 dark:bg-black/50 p-3 rounded-2xl border border-zinc-200 dark:border-zinc-800"><p className="text-[10px] font-bold text-zinc-500 uppercase mb-1">Diagnóstico</p><p className="text-xs text-zinc-600 dark:text-zinc-300 italic">"{m.diagnosis}"</p></div><div className="bg-zinc-50 dark:bg-black/50 p-3 rounded-2xl border border-zinc-200 dark:border-zinc-800"><p className="text-[10px] font-bold text-zinc-500 uppercase mb-1">Estratégia</p><p className="text-xs text-zinc-600 dark:text-zinc-300 italic">"{m.strategy}"</p></div></div>}</div>}</div>)}</div>
      </div>
      <Modal isOpen={cons.open} onClose={() => setCons({ ...cons, open: false })} title="Você realmente aprendeu?"><form onSubmit={e => { e.preventDefault(); if (cons.diag && cons.strat) { consolidateMistake(cons.id, cons.diag, cons.strat); setCons({ ...cons, open: false }); alert("Erro consolidado! +100 XP"); } }} className="space-y-4"><div className="bg-zinc-100 dark:bg-zinc-900/50 p-3 rounded-2xl border border-zinc-200 dark:border-zinc-800 mb-4"><p className="text-xs text-zinc-500 font-bold uppercase mb-1">Erro original</p><p className="text-sm text-zinc-600 dark:text-zinc-300 line-clamp-2">{cons.d}</p></div><div><label className="text-xs text-[#4d4dff] font-bold uppercase">Diagnóstico</label><textarea required className="w-full bg-zinc-50 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-sm text-zinc-900 dark:text-white h-20 outline-none focus:border-[#1100ab]" value={cons.diag} onChange={e => setCons({ ...cons, diag: e.target.value })} /></div><div><label className="text-xs text-green-400 font-bold uppercase">Estratégia</label><textarea required className="w-full bg-zinc-50 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-sm text-zinc-900 dark:text-white h-20 outline-none focus:border-green-500" value={cons.strat} onChange={e => setCons({ ...cons, strat: e.target.value })} /></div><Button type="submit" className="w-full mt-2">Confirmar Aprendizado</Button></form></Modal>
    </div>
  );
};
...
const GoalsView = () => {
  ...
  if (viewSub) {
    ...
    return (
      <div className="space-y-6 animate-fadeIn pb-24 md:pb-0"><button onClick={() => setViewSub(null)} className="flex items-center gap-2 text-zinc-400 hover:text-zinc-900 dark:hover:text-white"><ArrowLeft size={20} /> Voltar</button>
        <header className="flex justify-between items-end border-b border-zinc-200 dark:border-zinc-800 pb-6"><div><span className="text-xs font-bold uppercase px-2 py-1 rounded text-white mb-2 inline-block" style={{ backgroundColor: s.color }}>{s.name}</span><h1 className="text-3xl font-bold text-zinc-900 dark:text-white">Conteúdo</h1><div className="flex items-center gap-4 mt-2"><div className="flex items-center gap-2 text-zinc-500 text-sm"><CheckSquare size={16} className="text-[#1100ab]" /><span>{comp} / {tot} Tópicos</span></div><div className="flex items-center gap-2 text-zinc-500 text-sm"><Percent size={16} className="text-[#1100ab]" /><span>{tot ? Math.round((comp / tot) * 100) : 0}% Concluído</span></div></div></div><div className="text-right text-2xl font-bold text-zinc-900 dark:text-white">{s.goalHours}h <span className="text-xs text-zinc-500 block font-normal">Meta Semanal</span></div></header>
        <div className="bg-zinc-100 dark:bg-zinc-900/50 p-4 rounded-2xl border border-zinc-200 dark:border-zinc-800 flex gap-4 items-center"><div className="flex-1"><label className="text-xs text-zinc-500 uppercase font-bold mb-1">Novo Tema</label><input className="w-full bg-white dark:bg-black border border-zinc-300 dark:border-zinc-700 rounded-2xl p-2 text-zinc-900 dark:text-white" value={newTheme} onChange={e => setNewTheme(e.target.value)} onKeyDown={e => e.key === 'Enter' && newTheme && addTheme(viewSub, newTheme) && setNewTheme("")} /></div><Button onClick={() => newTheme && addTheme(viewSub, newTheme) && setNewTheme("")}>Adicionar</Button></div>
        <div className="space-y-6">{sTh.map(t => { const p = t.items.length ? Math.round((t.items.filter(i => i.completed).length / t.items.length) * 100) : 0; return (<Card key={t.id} className="relative group transition-all duration-300"><div className="flex justify-between items-start mb-4"><div className="flex gap-3 w-full"><button onClick={() => setCol({ ...col, [t.id]: !col[t.id] })} className="text-zinc-500">{col[t.id] ? <ChevronRight /> : <ChevronDown />}</button><div className="flex-1"><h3 onClick={() => setCol({ ...col, [t.id]: !col[t.id] })} className="text-xl font-bold text-zinc-900 dark:text-white cursor-pointer select-none">{t.title}</h3><div className="flex items-center gap-2 mt-1"><div className="h-1.5 w-24 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden"><div className="h-full bg-[#1100ab]" style={{ width: `${p}%` }} /></div><span className="text-xs text-zinc-500">{p}%</span></div></div></div><button onClick={() => deleteTheme(t.id)} className="text-zinc-400 hover:text-red-500"><Trash2 size={16} /></button></div>{!col[t.id] && <div className="space-y-2 mb-4 animate-fadeIn pl-8">{t.items.map(i => <div key={i.id} className="flex gap-3 p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800/50 rounded-2xl group/item"><button onClick={() => toggleThemeItem(t.id, i.id)} className={`w-5 h-5 rounded border flex items-center justify-center ${i.completed ? 'bg-[#1100ab] border-[#1100ab]' : 'border-zinc-400 dark:border-zinc-600'}`}>{i.completed && <CheckSquare size={14} className="text-white" />}</button><span className={`text-sm flex-1 ${i.completed ? 'text-zinc-500 line-through' : 'text-zinc-700 dark:text-zinc-200'}`}>{i.text}</span><button onClick={() => deleteThemeItem(t.id, i.id)} className="opacity-0 group-hover/item:opacity-100 text-zinc-400 hover:text-red-500"><X size={14} /></button></div>)}<form onSubmit={e => { e.preventDefault(); if (e.target.item.value) { addThemeItem(t.id, e.target.item.value); e.target.reset(); } }} className="flex gap-2 mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800/50"><input name="item" className="flex-1 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-2xl px-3 py-1.5 text-sm text-zinc-900 dark:text-white" placeholder="Tópico (Ctrl+V para lista)..." onPaste={e => { const d = e.clipboardData.getData('text'); if (d.includes('\n')) { e.preventDefault(); d.split('\n').map(l => l.trim()).filter(l => l).forEach(l => addThemeItem(t.id, l)); } }} /><button type="submit" className="p-1.5 bg-zinc-200 dark:bg-zinc-800 hover:bg-[#1100ab] text-zinc-900 dark:text-white hover:text-white rounded"><Plus size={16} /></button></form></div>}</Card>) })}</div>
      </div>
    );
  }

  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0"><header className="flex justify-between items-center mb-4"><h1 className="text-2xl font-bold text-zinc-900 dark:text-white">Metas Semanais</h1><Button onClick={() => setModal(true)}><Plus size={18} /> Nova</Button></header>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">{subjects.map(s => { const { h, p } = getProg(s.id, s.goalHours); return (<Card key={s.id} className="relative overflow-hidden group cursor-pointer hover:border-[#1100ab]/50"><div onClick={() => setViewSub(s.id)}><div className="absolute top-0 left-0 w-full h-1" style={{ backgroundColor: s.color }} /><div className="mb-4"><h3 className="text-xl font-bold text-zinc-900 dark:text-white">{s.name}</h3>{edit.id !== s.id && <p className="text-sm text-zinc-500">Meta: {s.goalHours}h</p>}</div>{edit.id !== s.id && (<><div className="mb-2 flex justify-between items-end"><span className="text-3xl font-bold text-zinc-900 dark:text-white">{h}h</span><span className="text-sm font-medium" style={{ color: s.color }}>{p}%</span></div><div className="w-full h-3 bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden"><div className="h-full rounded-full transition-all duration-1000" style={{ width: `${p}%`, backgroundColor: s.color }} /></div></>)}</div><div className="absolute top-4 right-4 flex gap-2">{edit.id === s.id ? (<div className="flex gap-2 bg-black/80 p-1 rounded-2xl"><input type="number" className="w-16 bg-black border border-zinc-600 rounded-2xl px-1 text-sm text-white" value={edit.val} onChange={e => setEdit({ ...edit, val: e.target.value })} autoFocus /><button onClick={() => { updateSubject(s.id, edit.val / 60); setEdit({ id: null, val: "" }) }} className="text-green-400 text-xs font-bold">OK</button></div>) : (<><button onClick={e => { e.stopPropagation(); setEdit({ id: s.id, val: Math.round(s.goalHours * 60) }) }} className="p-2 rounded-2xl bg-zinc-100 dark:bg-[#18181B]"><Settings size={16} className="text-zinc-400" /></button><button onClick={e => { e.stopPropagation(); deleteSubject(s.id) }} className="p-2 rounded-2xl bg-zinc-100 dark:bg-[#18181B]"><Trash2 size={16} className="text-zinc-400 hover:text-red-500" /></button></>)}</div></Card>) })}</div>
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Nova Matéria"><form onSubmit={e => { e.preventDefault(); addSubject(f.name, f.color, f.goal / 60); setModal(false); setF({ name: "", goal: 60, color: "#8b5cf6" }); }} className="space-y-4"><div><label className="text-sm text-zinc-500">Nome</label><input required className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-2 text-zinc-900 dark:text-white" value={f.name} onChange={e => setF({ ...f, name: e.target.value })} /></div><div><label className="text-sm text-zinc-500">Meta Semanal (minutos)</label><input required type="number" className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-2 text-zinc-900 dark:text-white" value={f.goal} onChange={e => setF({ ...f, goal: e.target.value })} /></div><div><label className="text-sm text-zinc-500">Cor</label><div className="flex gap-2 mt-2">{['#8b5cf6', '#10b981', '#f59e0b', '#ec4899', '#3b82f6', '#ef4444'].map(c => <div key={c} onClick={() => setF({ ...f, color: c })} className={`w-8 h-8 rounded-full cursor-pointer border-2 ${f.color === c ? 'border-zinc-900 dark:border-white' : 'border-transparent'}`} style={{ backgroundColor: c }} />)}</div></div><Button type="submit" className="w-full mt-4">Criar</Button></form></Modal>
    </div>
  );
};
...
const StatsView = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0"><h1 className="text-2xl font-bold text-zinc-900 dark:text-white mb-6">Central de Dados</h1>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">{[{ l: 'Tempo Total', v: `${(sessions.reduce((a, s) => a + s.minutes, 0) / 60).toFixed(1)}h`, c: '#1100ab' }, { l: 'Questões', v: totQ, c: 'blue-500' }, { l: 'Precisão Global', v: `${totQ > 0 ? Math.round(((totQ - totE) / totQ) * 100) : 0}%`, c: 'emerald-500' }, { l: 'Tópicos Feitos', v: themes.reduce((a, t) => a + t.items.filter(i => i.completed).length, 0), c: 'yellow-500' }].map((x, i) => <Card key={i} className={`flex flex-col gap-1 border-l-4 border-l-[${x.c}]`}><span className="text-xs text-zinc-500 uppercase font-bold">{x.l}</span><span className="text-2xl font-bold text-zinc-900 dark:text-white">{x.v}</span></Card>)}</div>
      <Card><h3 className="text-zinc-900 dark:text-white font-semibold mb-4 flex items-center gap-2"><Trophy size={18} className="text-yellow-500" /> Destaques de Performance</h3>
        {hi ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-sm">
          ...
          ].map((x, i) => <div key={i} className="bg-zinc-50 dark:bg-zinc-900/50 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800 flex flex-col justify-between h-full"><p className="text-xs text-zinc-500 uppercase font-bold flex items-center gap-1"><x.i size={12} className={`text-${x.c.replace('#', '')}`} /> {x.l}</p><p className="text-zinc-900 dark:text-white font-bold truncate mt-1 text-lg">{x.v}</p><p className={`text-xs text-${x.c.replace('#', '')} mt-1`}>{x.s}</p></div>)}
        </div> : <p className="text-zinc-500">Estude mais para gerar destaques.</p>}</Card>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card className="min-h-[300px]"><h3 className="text-zinc-900 dark:text-white font-semibold mb-6 flex items-center gap-2"><Activity size={18} className="text-[#1100ab]" /> Volume de Questões</h3><div className="h-[250px] w-full"><ResponsiveContainer><BarChart data={perfData}><CartesianGrid strokeDasharray="3 3" stroke="#52525b" opacity={0.3} vertical={false} /><XAxis dataKey="name" stroke="#555" tick={{ fontSize: 10 }} axisLine={false} /><Tooltip cursor={{ fill: '#222' }} contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} itemStyle={{ color: '#fff' }} /><Bar dataKey="Questões" fill="#1100ab" radius={[4, 4, 0, 0]} name="Feitas" /><Bar dataKey="Erros" fill="#ef4444" radius={[4, 4, 0, 0]} name="Erros" /></BarChart></ResponsiveContainer></div></Card><Card className="min-h-[300px]"><h3 className="text-zinc-900 dark:text-white font-semibold mb-6 flex items-center gap-2"><Calendar size={18} className="text-blue-500" /> Atividade Mensal</h3><div className="h-[250px] w-full"><ResponsiveContainer><BarChart data={monthlyData}><CartesianGrid strokeDasharray="3 3" stroke="#52525b" opacity={0.3} vertical={false} /><XAxis dataKey="name" stroke="#555" tick={{ fontSize: 10 }} interval={2} axisLine={false} /><Tooltip cursor={{ fill: '#222' }} contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} itemStyle={{ color: '#fff' }} /><Bar dataKey="minutes" fill="#3b82f6" radius={[2, 2, 0, 0]} name="Minutos" /></BarChart></ResponsiveContainer></div></Card></div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card className="min-h-[300px]"><h3 className="text-zinc-900 dark:text-white font-semibold mb-6 flex items-center gap-2"><Layers size={18} className="text-purple-500" /> Tópicos Finalizados</h3><div className="h-[250px] w-full flex items-center justify-center">{compTopics.length ? <ResponsiveContainer><PieChart><Pie data={compTopics} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">{compTopics.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} itemStyle={{ color: '#fff' }} /><Legend /></PieChart></ResponsiveContainer> : <p className="text-zinc-500 text-sm">Nenhum tópico concluído.</p>}</div></Card><Card className="min-h-[300px]"><h3 className="text-zinc-900 dark:text-white font-semibold mb-6 flex items-center gap-2"><AlertTriangle size={18} className="text-red-500" /> Questões Erradas por Matéria</h3><div className="h-[250px] w-full flex items-center justify-center">{wrnQ.length ? <ResponsiveContainer><PieChart><Pie data={wrnQ} cx="50%" cy="50%" innerRadius={40} outerRadius={80} paddingAngle={5} dataKey="value">{wrnQ.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} itemStyle={{ color: '#fff' }} /><Legend /></PieChart></ResponsiveContainer> : <p className="text-zinc-500 text-sm">Sem registros de erros.</p>}</div></Card></div>
      <div className="grid grid-cols-1 gap-6"><Card className="min-h-[300px]"><h3 className="text-zinc-900 dark:text-white font-semibold mb-6 flex items-center gap-2"><AlertTriangle size={18} className="text-orange-500" /> Ranking de Motivos de Erro</h3><div className="h-[250px] w-full"><ResponsiveContainer><BarChart data={misReasons} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#52525b" opacity={0.3} horizontal={false} /><XAxis type="number" stroke="#555" tick={{ fontSize: 10 }} /><YAxis dataKey="name" type="category" stroke="#555" tick={{ fontSize: 10 }} width={100} /><Tooltip cursor={{ fill: '#222' }} contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} itemStyle={{ color: '#fff' }} /><Bar dataKey="quantidade" fill="#f97316" radius={[0, 4, 4, 0]} barSize={20} /></BarChart></ResponsiveContainer></div></Card></div>
      <Card><h3 className="text-zinc-900 dark:text-white font-semibold mb-4 flex items-center gap-2"><InfinityIcon size={18} className="text-yellow-500" /> Roadmap de Consistência</h3><div className="flex flex-wrap gap-1">{heat.map((d, i) => <div key={i} title={`${d.date.toLocaleDateString()}: ${d.hasStudy ? 'Estudou' : 'Sem registro'}`} className={`w-3 h-3 rounded-sm transition-all hover:scale-125 ${d.hasStudy ? 'bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.5)]' : 'bg-zinc-300 dark:bg-zinc-800'}`}></div>)}</div><p className="text-xs text-zinc-500 mt-3">Cada quadrado representa um dia deste ano.</p></Card>
    </div>
  );
};
...
const AppLayout = () => {
  ...
  return (
    // CORREÇÃO: Removi 'bg-zinc-50 dark:bg-[#000000]' daqui. Agora é 'bg-transparent'.
    // A cor de fundo agora vem 100% do body (definido no index.css), evitando conflitos.
    <div className="flex h-screen bg-transparent text-zinc-900 dark:text-zinc-300 font-sans font-medium overflow-hidden"><button className="md:hidden fixed top-4 right-4 z-50 p-2 bg-white dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-2xl" onClick={() => setMenu(!menu)}>{menu ? <X /> : <Menu />}</button>{menu && <div className="fixed inset-0 bg-black/90 z-40 md:hidden" onClick={() => setMenu(false)} />}
      <aside className={`fixed inset-y-0 left-0 z-50 bg-white dark:bg-[#000000] border-r border-zinc-200 dark:border-zinc-900 flex flex-col transition-all duration-300 md:translate-x-0 ${menu ? 'translate-x-0' : '-translate-x-full'} ${col ? 'w-20' : 'w-64'}`}>
        <div className={`p-6 flex items-center ${col ? 'justify-center flex-col gap-4' : 'justify-between'}`}><div className="flex items-center gap-3"><div className="w-8 h-8 bg-gradient-to-br from-[#1100ab] to-blue-900 rounded-2xl flex items-center justify-center text-white font-bold flex-shrink-0">F</div>{!col && <span className="text-xl font-bold text-zinc-900 dark:text-white tracking-tight">Focus</span>}</div><button onClick={() => setCol(!col)} className="p-2 hover:bg-zinc-100 dark:hover:bg-[#09090b] rounded-xl text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors">{col ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}</button></div>
        <div className={`mx-4 mb-6 p-3 bg-zinc-50 dark:bg-[#09090b] border border-zinc-200 dark:border-zinc-800 rounded-3xl ${col ? 'flex justify-center' : ''}`}><div className={`flex items-center gap-3 ${col ? 'justify-center' : 'mb-2'}`}><div className={`w-8 h-8 rounded-full bg-gradient-to-br ${rk.b} flex items-center justify-center text-xs text-white font-bold`}>{userLevel.level}</div>{!col && <div className="flex-1 min-w-0"><p className="text-xs font-bold text-zinc-900 dark:text-white truncate">{userLevel.title}</p><p className="text-[10px] text-zinc-500">{userLevel.currentXP}/{xpN} XP</p></div>}</div>{!col && <div className="h-1.5 w-full bg-zinc-200 dark:bg-zinc-800 rounded-full overflow-hidden"><div className={`h-full bg-gradient-to-r ${rk.b}`} style={{ width: `${xpP}%` }} /></div>}</div>
      ...
      <main className={`flex-1 overflow-y-auto h-full p-4 md:p-8 bg-zinc-50 dark:bg-[#000000] transition-all duration-300 ${col ? 'md:ml-20' : 'md:ml-64'}`}><div className="max-w-6xl mx-auto h-full">{currentView === 'dashboard' && <DashboardView />} {currentView === 'focus' && <FocusView />} {currentView === 'calendar' && <CalendarTab />} {currentView === 'mistakes' && <MistakesView />} {currentView === 'goals' && <GoalsView />} {currentView === 'stats' && <StatsView />} {currentView === 'history' && <HistoryView />} {currentView === 'settings' && <SettingsView />}</div></main>
    </div>
  );
};

const App = () => (<FocusProvider><AppLayout /><style>{`.custom-scrollbar::-webkit-scrollbar{width:6px}.custom-scrollbar::-webkit-scrollbar-thumb{background-color:#555;border-radius:20px} .dark .custom-scrollbar::-webkit-scrollbar-thumb{background-color:#222} @keyframes fadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}.animate-fadeIn{animation:fadeIn 0.4s ease-out forwards}::selection{background-color:#1100ab;color:#fff}`}</style></FocusProvider>);
export default App;