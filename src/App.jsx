  ...
  const addSession = (minutes, notes, manualSubId = null, questions = 0, errors = 0) => {
    ...
    if (subject) {
        ...
        const previousWeeklyMins = sessions.reduce((acc, s) => {
            ...
            if (s.subjectId === sId && sDate >= startOfWeek) {
    ...
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="bg-gradient-to-br from-violet-900/40 to-[#18181B] border-violet-500/30 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div><h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">Trabalho Profundo <InfinityIcon size={18} className="text-violet-400"/></h3><p className="text-gray-300 mb-6 italic text-sm border-l-2 border-violet-500/50 pl-3">"A superficialidade não constrói impérios."</p></div>
          <Button onClick={() => setCurrentView('focus')} className="w-full py-4 text-lg shadow-lg shadow-violet-900/20 hover:shadow-violet-600/40 border border-white/5">Mergulhar no Foco <Zap size={20} className="fill-current"/></Button>
        </Card>
      </div>

      <Modal isOpen={isCountModalOpen} onClose={() => setIsCountModalOpen(false)} title="Configurar Meta"><form onSubmit={handleSaveCountdown} className="space-y-4"><div><label className="text-xs text-gray-500 font-bold uppercase">Nome da Meta</label><input type="text" placeholder="Ex: ENEM, Prova de Inglês..." className="w-full mt-1 bg-[#0F0F12] border border-gray-700 rounded-lg p-3 text-white outline-none focus:border-violet-500" value={countForm.title} onChange={e => setCountForm({...countForm, title: e.target.value})}/></div><div><label className="text-xs text-gray-500 font-bold uppercase">Data Alvo</label><input type="date" required className="w-full mt-1 bg-[#0F0F12] border border-gray-700 rounded-lg p-3 text-white outline-none focus:border-violet-500" value={countForm.date} onChange={e => setCountForm({...countForm, date: e.target.value})}/></div><div className="pt-2 flex gap-2"><Button type="button" variant="secondary" onClick={() => { setCountdown({date: null, title: ''}); setIsCountModalOpen(false); }} className="flex-1">Limpar</Button><Button type="submit" className="flex-[2]">Salvar Data</Button></div></form></Modal>
    </div>
  );
};
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fadeIn pb-24 md:pb-0">
      ...
      <Modal isOpen={isFinishModalOpen} onClose={() => setIsFinishModalOpen(false)} title="Resumo da Sessão">
        <form onSubmit={handleConfirmSession} className="space-y-4">
          <div><label className="text-xs text-gray-500 font-bold uppercase flex items-center gap-2"><FileText size={14}/> O que você estudou?</label><textarea className="w-full mt-1 bg-[#0F0F12] border border-gray-700 rounded-lg p-3 text-sm text-white h-24 outline-none focus:border-violet-500 transition-colors resize-none" placeholder="Descreva seu estudo..." value={finishForm.notes} onChange={e => setFinishForm({...finishForm, notes: e.target.value})}/></div>
          <div className="grid grid-cols-2 gap-4"><div><label className="text-xs text-gray-500 font-bold uppercase flex items-center gap-2"><Activity size={14}/> Questões Feitas</label><input type="number" min="0" className="w-full mt-1 bg-[#0F0F12] border border-gray-700 rounded-lg p-3 text-white outline-none focus:border-blue-500" value={finishForm.questions} onChange={e => setFinishForm({...finishForm, questions: e.target.value})}/></div><div><label className="text-xs text-gray-500 font-bold uppercase flex items-center gap-2"><AlertTriangle size={14}/> Questões Erradas</label><input type="number" min="0" className="w-full mt-1 bg-[#0F0F12] border border-gray-700 rounded-lg p-3 text-white outline-none focus:border-red-500" value={finishForm.errors} onChange={e => setFinishForm({...finishForm, errors: e.target.value})}/></div></div>
      ...
      <Modal isOpen={isManualOpen} onClose={() => setIsManualOpen(false)} title="Registro Manual">
        <form onSubmit={handleManualSubmit} className="space-y-5">
          <div className="space-y-2"><label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Matéria</label><select required className="w-full bg-[#0F0F12] border border-zinc-800 rounded-xl p-3 text-white outline-none focus:border-violet-500 transition-colors" value={manualSubId} onChange={e => setManualSubId(e.target.value)}>{subjects.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
          <div className="space-y-2"><label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Tempo (minutos)</label><input required type="number" min="1" placeholder="Ex: 60" className="w-full bg-[#0F0F12] border border-zinc-800 rounded-xl p-3 text-white outline-none focus:border-violet-500 transition-colors" value={manualMinutes} onChange={e => setManualMinutes(e.target.value)}/></div>
          <div className="space-y-2"><label className="text-xs font-bold text-zinc-500 uppercase tracking-wider">Diário da Sessão</label><textarea className="w-full bg-[#0F0F12] border border-zinc-800 rounded-xl p-3 text-white h-28 outline-none focus:border-violet-500 transition-colors resize-none text-sm" placeholder="Resumo..." value={manualNotes} onChange={e => setManualNotes(e.target.value)}/></div>
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">{mistakes.length === 0 ? (<div className="text-center py-10 opacity-50"><CheckCircle size={40} className="mx-auto mb-2"/><p>Vazio.</p></div>) : (mistakes.map(m => (<div key={m.id} className={`border rounded-xl p-5 relative group transition-all duration-300 ${m.consolidated ? 'bg-zinc-900/50 border-zinc-800 opacity-75' : 'bg-[#18181B] border-gray-800 hover:border-gray-700'}`}>{m.consolidated && (<div className="absolute top-4 right-12 bg-green-500/10 text-green-500 text-[10px] font-bold px-2 py-1 rounded border border-green-500/20 flex items-center gap-1 select-none"><CheckCircle size={10} /> ERRO APRENDIDO</div>)}<div className="absolute top-4 right-4 flex gap-2">{!m.consolidated && (<button onClick={() => openConsolidation(m)} title="Marcar como aprendido" className="text-gray-600 hover:text-green-500 opacity-0 group-hover:opacity-100 transition-opacity"><CheckCircle size={18}/></button>)}<button onClick={() => deleteMistake(m.id)} className="text-gray-600 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={18}/></button></div><div className="flex items-center gap-3 mb-3"><span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase text-white" style={{backgroundColor: subjects.find(s => s.id === m.subjectId)?.color}}>{subjects.find(s => s.id === m.subjectId)?.name}</span><span className="text-xs font-medium px-2 py-0.5 rounded-full bg-red-500/10 text-red-400 border border-red-500/20">{m.reason}</span></div><div className="grid md:grid-cols-2 gap-6"><div className="space-y-1"><p className="text-xs text-red-400 font-bold uppercase">Erro</p><p className="text-gray-300 text-sm">{m.description}</p></div><div className="space-y-1 md:border-l md:border-gray-800 md:pl-6"><p className="text-xs text-green-400 font-bold uppercase">Solução</p><p className="text-gray-300 text-sm">{m.solution}</p></div></div>{m.consolidated && (<div className="mt-4 pt-4 border-t border-gray-800/50"><button onClick={() => toggleExpand(m.id)} className="text-xs text-violet-400 hover:text-white flex items-center gap-1 transition-colors">{expandedIds.includes(m.id) ? 'Ocultar reflexão' : 'Ver minha reflexão'} <ChevronDown size={14} className={`transition-transform ${expandedIds.includes(m.id) ? 'rotate-180' : ''}`}/></button>{expandedIds.includes(m.id) && (<div className="mt-3 grid md:grid-cols-2 gap-4 animate-fadeIn"><div className="bg-zinc-950/50 p-3 rounded border border-zinc-800"><p className="text-[10px] font-bold text-zinc-500 uppercase mb-1">Diagnóstico</p><p className="text-xs text-gray-300 italic">"{m.diagnosis}"</p></div><div className="bg-zinc-950/50 p-3 rounded border border-zinc-800"><p className="text-[10px] font-bold text-zinc-500 uppercase mb-1">Estratégia</p><p className="text-xs text-gray-300 italic">"{m.strategy}"</p></div></div>)}</div>)}</div>)))}</div>
      </div>
      <Modal isOpen={consModal.isOpen} onClose={() => setConsModal({...consModal, isOpen: false})} title="Você realmente aprendeu?"><form onSubmit={handleConsolidate} className="space-y-4"><div className="bg-zinc-900/50 p-3 rounded border border-zinc-800 mb-4"><p className="text-xs text-gray-500 font-bold uppercase mb-1">Erro original</p><p className="text-sm text-gray-300 line-clamp-2">{consModal.mDesc}</p></div><div><label className="text-xs text-violet-400 font-bold uppercase">Diagnóstico</label><p className="text-[10px] text-gray-500 mb-2">Por que eu errei antes? (Ex: Falta de atenção, pegadinha)</p><textarea required className="w-full bg-[#0F0F12] border border-gray-700 rounded-lg p-3 text-sm text-white h-20 outline-none focus:border-violet-500 transition-colors" value={consForm.diag} onChange={e => setConsForm({...consForm, diag: e.target.value})}/></div><div><label className="text-xs text-green-400 font-bold uppercase">Estratégia</label><p className="text-[10px] text-gray-500 mb-2">Como não vou errar mais? (Ex: Mnemônico, regra)</p><textarea required className="w-full bg-[#0F0F12] border border-gray-700 rounded-lg p-3 text-sm text-white h-20 outline-none focus:border-green-500 transition-colors" value={consForm.strat} onChange={e => setConsForm({...consForm, strat: e.target.value})}/></div><Button type="submit" className="w-full mt-2">Confirmar Aprendizado</Button></form></Modal>
    </div>
  );
};
...
const StatsView = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4"><Card className="bg-gradient-to-br from-orange-500/10 to-[#18181B] border-orange-500/20"><div className="flex items-center gap-3 mb-2"><Flame className="text-orange-500" size={20} /><h3 className="text-gray-400 text-xs font-bold uppercase">Recorde Histórico</h3></div><p className="text-3xl font-bold text-white">{maxStreak} <span className="text-sm font-normal text-gray-500">dias seguidos</span></p></Card><Card className="bg-gradient-to-br from-emerald-500/10 to-[#18181B] border-emerald-500/20"><div className="flex items-center gap-3 mb-2"><Target className="text-emerald-500" size={20} /><h3 className="text-gray-400 text-xs font-bold uppercase">Mais Estudada</h3></div><p className="text-xl font-bold text-white truncate">{bestSubject ? bestSubject.name : "---"}</p><p className="text-xs text-emerald-400">{bestSubject ? (bestSubject.totalMins / 60).toFixed(1) : 0} horas totais</p></Card><Card className="bg-gradient-to-br from-red-500/10 to-[#18181B] border-red-500/20"><div className="flex items-center gap-3 mb-2"><AlertTriangle className="text-red-500" size={20} /><h3 className="text-gray-400 text-xs font-bold uppercase">Atenção Necessária</h3></div><p className="text-xl font-bold text-white truncate">{worstSubject ? worstSubject.name : "---"}</p><p className="text-xs text-red-400">{worstSubject ? (worstSubject.totalMins / 60).toFixed(1) : 0} horas totais</p></Card></div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card className="min-h-[300px]"><h3 className="text-white font-semibold mb-6 flex items-center gap-2"><BarChart2 size={18} className="text-violet-500"/> Performance Semanal</h3><div className="h-[200px] w-full"><ResponsiveContainer><BarChart data={weeklyChartData}><CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false}/><XAxis dataKey="name" stroke="#555" tick={{fontSize:10}} axisLine={false} tickLine={false}/><Tooltip cursor={{fill:'#222'}} contentStyle={{backgroundColor:'#18181B',borderColor:'#333', color:'#fff'}}/><Bar dataKey="minutos" fill="#8b5cf6" radius={[4,4,0,0]} /></BarChart></ResponsiveContainer></div></Card><Card className="min-h-[300px]"><h3 className="text-white font-semibold mb-6 flex items-center gap-2"><Calendar size={18} className="text-blue-500"/> Visão Mensal</h3><div className="h-[200px] w-full"><ResponsiveContainer><BarChart data={monthlyData}><CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false}/><XAxis dataKey="name" stroke="#555" tick={{fontSize:10}} interval={2} axisLine={false} tickLine={false}/><Tooltip cursor={{fill:'#222'}} contentStyle={{backgroundColor:'#18181B',borderColor:'#333', color:'#fff'}}/><Bar dataKey="minutos" fill="#3b82f6" radius={[2,2,0,0]} /></BarChart></ResponsiveContainer></div></Card></div>
      <Card><h3 className="text-white font-semibold mb-4 flex items-center gap-2"><InfinityIcon size={18} className="text-yellow-500"/> Roadmap de Consistência</h3><div className="flex flex-wrap gap-1">{heatmapData.map((day, index) => (<div key={index} title={`${day.date.toLocaleDateString()}: ${day.hasStudy ? 'Estudou' : 'Sem registro'}`} className={`w-3 h-3 rounded-sm transition-all hover:scale-125 ${day.hasStudy ? 'bg-green-500 shadow-[0_0_5px_rgba(34,197,94,0.5)]' : 'bg-[#27272A]'}`}></div>))}</div><p className="text-xs text-gray-500 mt-3">Cada quadrado representa um dia deste ano. Quadrados acesos indicam dias com estudo registrado.</p></Card>
    </div>
  );
};
...
const ReportView = () => {
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0"><button onClick={()=>setCurrentView('history')} className="flex items-center gap-2 text-gray-400 hover:text-white mb-2"><ArrowLeft size={18}/> Voltar</button><header className="flex flex-col md:flex-row justify-between items-center gap-6"><div><h1 className="text-2xl font-bold text-white capitalize">{selectedHistoryDate.toLocaleDateString()}</h1><p className="text-gray-400">Relatório</p></div><div className="w-32 h-32 rounded-full border-4 border-violet-500/20 flex flex-col items-center justify-center"><span className="text-2xl font-mono font-bold text-white">{Math.floor(t/60)}h {t%60}m</span><span className="text-xs uppercase text-gray-500">Total</span></div></header>
      <div className="space-y-4">{daily.map(s=>{ const sub=subjects.find(x=>x.id===s.subjectId)||{name:'-',color:'#555'}; const accuracy = s.questions > 0 ? Math.round(((s.questions - s.errors) / s.questions) * 100) : 0; return ( <Card key={s.id} className="relative overflow-hidden"><div className="absolute left-0 top-0 bottom-0 w-1" style={{backgroundColor:sub.color}}/><div className="flex justify-between items-start"><div className="flex-1 pl-3"><div className="flex items-center gap-2 mb-1"><span className="text-[10px] font-bold uppercase text-white px-1.5 rounded" style={{backgroundColor:sub.color}}>{sub.name}</span><span className="text-xs text-gray-500">{new Date(s.date).toLocaleTimeString().slice(0,5)}</span></div><p className="text-sm text-gray-300">{s.notes||"Sem notas."}</p>{s.questions > 0 && (<div className="mt-3 flex gap-3 text-xs"><span className="flex items-center gap-1 bg-blue-500/10 text-blue-400 px-2 py-1 rounded border border-blue-500/20 font-bold"><Activity size={12}/> {s.questions} Questões</span><span className={`flex items-center gap-1 px-2 py-1 rounded border font-bold ${s.errors > 0 ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-green-500/10 text-green-400 border-green-500/20'}`}>{s.errors > 0 ? <AlertTriangle size={12}/> : <CheckCircle size={12}/>} {s.errors} Erros</span><span className="flex items-center gap-1 bg-violet-500/10 text-violet-400 px-2 py-1 rounded border border-violet-500/20 font-bold"><Percent size={12}/> {accuracy}% Acerto</span></div>)}</div><span className="font-bold text-white">{s.minutes}m</span></div></Card> )})}</div>
...
export default App;import React, { useState, useEffect, useContext, createContext, useMemo, useRef } from 'react';
import { createPortal } from 'react-dom';
import { 
  LayoutDashboard, Zap, Target, BarChart2, Play, Pause, Coffee, RotateCcw, 
  CheckCircle, Plus, Clock, Flame, Settings, BookOpen, Quote, Trash2, Menu, X, 
