...
const StatsView = () => {
  ...
  const hi = useMemo(() => {
    ...
    return { ms: agg.sort((a, b) => b.min - a.min)[0], ls: agg[agg.length - 1], mt: [...agg].sort((a, b) => b.t - a.t)[0], lt: [...agg].sort((a, b) => a.t - b.t)[0], mq: [...agg].sort((a, b) => b.q - a.q)[0], mc: [...agg].sort((a, b) => (b.q - b.e) - (a.q - a.e))[0], me: [...agg].sort((a, b) => b.e - a.e)[0] };
  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0"><h1 className="text-2xl font-bold text-white mb-6">Central de Dados</h1>
      ...
      <Card><h3 className="text-white font-semibold mb-4 flex items-center gap-2"><Trophy size={18} className="text-yellow-500" /> Destaques de Performance</h3>
        {hi ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-sm">{[{ l: 'Mais Estudada', v: hi.ms.name, s: `${Math.round(hi.ms.min / 60)}h`, i: Clock, c: '#1100ab' }, { l: 'Menos Estudada', v: hi.ls.name, s: `${Math.round(hi.ls.min / 60)}h`, i: Clock, c: 'zinc-400' }, { l: 'Mais Tópicos', v: hi.mt.name, s: `${hi.mt.t} feitos`, i: CheckSquare, c: 'emerald-500' }, { l: 'Menos Tópicos', v: hi.lt.name, s: `${hi.lt.t} feitos`, i: CheckSquare, c: 'zinc-400' }, { l: 'Mais Questões', v: hi.mq.name, s: `${hi.mq.q} total`, i: Activity, c: 'blue-500' }, { l: 'Mais Acertos', v: hi.mc.name, s: `${hi.mc.q - hi.mc.e} certas`, i: CheckCircle, c: 'emerald-500' }].map((x, i) => <div key={i} className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800"><p className="text-xs text-zinc-500 uppercase font-bold flex items-center gap-1"><x.i size={12} /> {x.l}</p><p className="text-white font-bold truncate">{x.v}</p><p className={`text-xs text-${x.c.replace('#', '')}`}>{x.s}</p></div>)}<div className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-800 flex flex-col justify-center"><p className="text-xs text-zinc-500 uppercase font-bold flex items-center gap-1"><AlertTriangle size={12} /> Mais Erros</p><p className="text-white font-bold truncate text-lg mt-1">{hi.me.name}</p><p className="text-sm text-red-500 font-bold">{hi.me.e} erradas</p></div></div> : <p className="text-zinc-500">Estude mais para gerar destaques.</p>}</Card>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card className="min-h-[300px]"><h3 className="text-white font-semibold mb-6 flex items-center gap-2"><Activity size={18} className="text-[#1100ab]" /> Volume de Questões</h3><div className="h-[250px] w-full"><ResponsiveContainer><BarChart data={perfData}><CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} /><XAxis dataKey="name" stroke="#555" tick={{ fontSize: 10 }} axisLine={false} /><Tooltip cursor={{ fill: '#222' }} contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} /><Bar dataKey="Questões" fill="#1100ab" radius={[4, 4, 0, 0]} name="Feitas" /><Bar dataKey="Erros" fill="#ef4444" radius={[4, 4, 0, 0]} name="Erros" /></BarChart></ResponsiveContainer></div></Card><Card className="min-h-[300px]"><h3 className="text-white font-semibold mb-6 flex items-center gap-2"><Calendar size={18} className="text-blue-500" /> Atividade Mensal</h3><div className="h-[250px] w-full"><ResponsiveContainer><BarChart data={monthlyData}><CartesianGrid strokeDasharray="3 3" stroke="#27272a" vertical={false} /><XAxis dataKey="name" stroke="#555" tick={{ fontSize: 10 }} interval={2} axisLine={false} /><Tooltip cursor={{ fill: '#222' }} contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} /><Bar dataKey="minutes" fill="#3b82f6" radius={[2, 2, 0, 0]} name="Minutos" /></BarChart></ResponsiveContainer></div></Card></div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6"><Card className="min-h-[300px]"><h3 className="text-white font-semibold mb-6 flex items-center gap-2"><Layers size={18} className="text-purple-500" /> Tópicos Finalizados</h3><div className="h-[250px] w-full flex items-center justify-center">{compTopics.length ? <ResponsiveContainer><PieChart><Pie data={compTopics} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">{compTopics.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} /><Legend /></PieChart></ResponsiveContainer> : <p className="text-zinc-500 text-sm">Nenhum tópico concluído.</p>}</div></Card><Card className="min-h-[300px]"><h3 className="text-white font-semibold mb-6 flex items-center gap-2"><AlertTriangle size={18} className="text-red-500" /> Questões Erradas por Matéria</h3><div className="h-[250px] w-full flex items-center justify-center">{wrnQ.length ? <ResponsiveContainer><PieChart><Pie data={wrnQ} cx="50%" cy="50%" innerRadius={40} outerRadius={80} paddingAngle={5} dataKey="value">{wrnQ.map((e, i) => <Cell key={i} fill={e.color} />)}</Pie><Tooltip contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} /><Legend /></PieChart></ResponsiveContainer> : <p className="text-zinc-500 text-sm">Sem registros de erros.</p>}</div></Card></div>
      <div className="grid grid-cols-1 gap-6"><Card className="min-h-[300px]"><h3 className="text-white font-semibold mb-6 flex items-center gap-2"><AlertTriangle size={18} className="text-orange-500" /> Ranking de Motivos de Erro</h3><div className="h-[250px] w-full"><ResponsiveContainer><BarChart data={misReasons} layout="vertical"><CartesianGrid strokeDasharray="3 3" stroke="#27272a" horizontal={false} /><XAxis type="number" stroke="#555" tick={{ fontSize: 10 }} /><YAxis dataKey="name" type="category" stroke="#555" tick={{ fontSize: 10 }} width={100} /><Tooltip cursor={{ fill: '#222' }} contentStyle={{ backgroundColor: '#09090b', borderColor: '#333', color: '#fff' }} /><Bar dataKey="quantidade" fill="#f97316" radius={[0, 4, 4, 0]} barSize={20} /></BarChart></ResponsiveContainer></div></Card></div>
...
const AppLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-[#000000] text-zinc-300 font-sans font-medium overflow-hidden"><button className="md:hidden fixed top-4 right-4 z-50 p-2 bg-[#09090b] border border-zinc-800 rounded-2xl" onClick={() => setMenu(!menu)}>{menu ? <X /> : <Menu />}</button>{menu && <div className="fixed inset-0 bg-black/90 z-40 md:hidden" onClick={() => setMenu(false)} />}
      ...
      <main className={`flex-1 overflow-y-auto h-full p-4 md:p-8 bg-[#000000] transition-all duration-300 ${col ? 'md:ml-20' : 'md:ml-64'}`}><div className="max-w-6xl mx-auto h-full">{currentView === 'dashboard' && <DashboardView />} {currentView === 'focus' && <FocusView />} {currentView === 'calendar' && <CalendarTab />} {currentView === 'mistakes' && <MistakesView />} {currentView === 'goals' && <GoalsView />} {currentView === 'stats' && <StatsView />} {currentView === 'settings' && <SettingsView />}</div></main>
    </div>
  );
};
