...
const DEFAULT_SCHEDULE = Array.from({ length: 7 }).map((_, i) => ({ dayIndex: i, material: '', minMinutes: 30, skills: [], notes: '' }));
...
export const LanguageProvider = ({ children }) => {
  ...
  const addLanguageSession = (minutes, sessionSummary, skills, materials, date = new Date().toISOString()) => {
    ...
    // Corrigido: Usando o nome nativo do estado, setActiveLanguageSessions gerava Uncaught TypeError.
  ...
  const getTheme = () => {
    if (activeLanguage === 'EN') return { id: 'EN', flag: '🇺🇸', name: 'Inglês', colors: { primary: '#1e3a8a', secondary: '#3b82f6', tertiary: '#93c5fd', quaternary: '#bfdbfe' }, classes: { bg: 'bg-blue-50 dark:bg-[#0a192f]', border: 'border-blue-200 dark:border-blue-900', text: 'text-blue-900 dark:text-blue-100', button: 'bg-blue-900 hover:bg-blue-800', highlight: 'border-blue-500 bg-blue-100 dark:bg-blue-900/30' } };
    if (activeLanguage === 'ES') return { id: 'ES', flag: '🇪🇸', name: 'Espanhol', colors: { primary: '#ca8a04', secondary: '#eab308', tertiary: '#fde047', quaternary: '#fef08a' }, classes: { bg: 'bg-yellow-50 dark:bg-[#2b2204]', border: 'border-yellow-200 dark:border-yellow-900', text: 'text-yellow-900 dark:text-yellow-100', button: 'bg-yellow-600 hover:bg-yellow-500', highlight: 'border-yellow-500 bg-yellow-100 dark:bg-yellow-900/30' } };
    if (activeLanguage === 'DE') return { id: 'DE', flag: '🇩🇪', name: 'Alemão', colors: { primary: '#991b1b', secondary: '#ef4444', tertiary: '#fca5a5', quaternary: '#fecaca' }, classes: { bg: 'bg-red-50 dark:bg-[#2e0909]', border: 'border-red-200 dark:border-red-900', text: 'text-red-900 dark:text-red-100', button: 'bg-red-800 hover:bg-red-700', highlight: 'border-red-500 bg-red-100 dark:bg-red-900/30' } };
    return null;
  };
  ...
  return (
    <LanguageContext.Provider value={{ 
      ...
      languageSessions, setLanguageSessions, activeLanguageSessions, 
      addLanguageSession, deleteLanguageSession, 
      ...
      getTheme, deleteLanguageSessionsByDate, resetLanguageData
    }}>
      {children}
