...
import { 
    ...
    CornerDownRight, AlertTriangle, BookOpen, X 
} from 'lucide-react';
...
export const FocusView = () => {
  ...
  const handleAddSubTask = (taskId) => {
      const text = window.prompt("Nova subtarefa:");
      if (text && text.trim()) {
          ...
          <Card className="h-full flex flex-col min-h-[300px]">
            {isCurrentSubCursinho ? (
                <div className="flex flex-col items-center justify-center h-full text-center text-zinc-400 p-6 flex-1">
                    ...
                    <p className="text-sm font-medium">As tarefas não estão disponíveis para Cursinho.</p>
                </div>
            ) : (
                <>
        ...
        <form onSubmit={manual} className="space-y-5">
            ...
            <div className="space-y-2">
                ...
                <input required type="number" min="1" 
                    className="w-full bg-zinc-100 dark:bg-black border border-zinc-200 rounded-2xl p-3 outline-none focus:border-primary" 
                    value={mForm.t} 
