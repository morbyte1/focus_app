    ...
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0">
      ...
      <Modal isOpen={cons.open} onClose={() => setCons({ ...cons, open: false })} title="Você realmente aprendeu?">
        <form onSubmit={e => { e.preventDefault(); if (cons.diag && cons.strat) { consolidateMistake(cons.id, cons.diag, cons.strat); setCons({ ...cons, open: false }); alert("Erro consolidado! +100 XP"); } }} className="space-y-4">
          ...
          <div><label className="text-xs text-primary-light font-bold uppercase">Diagnóstico</label><textarea required className="w-full bg-zinc-50 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-sm text-zinc-900 dark:text-white h-20 outline-none focus:border-primary" value={cons.diag} onChange={e => setCons({ ...cons, diag: e.target.value })} /></div>
