  ...
  return (
    <div className="space-y-6 animate-fadeIn pb-24 md:pb-0">
        ...
        <div className="flex flex-col md:flex-row gap-6 items-center relative z-10">
          ...
          <div className="flex-1 w-full">
            <div className="flex justify-between items-end mb-2">
              ...
              <div className="hidden sm:block text-white dark:text-[#4d4dff] font-bold">{userLevel.currentXP} <span className="text-zinc-200 dark:text-zinc-500">/ {xpNext} XP</span></div>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[{ l: 'Hoje', v: `${kpiData.todayMinutes} min`, i: Zap, c: 'yellow-500' }, { l: 'Total', v: `${kpiData.totalHours} h`, i: Clock, c: '[#1100ab]', ic: '[#4d4dff]' }, { l: 'Sequência', v: `${kpiData.streak} dias`, i: Flame, c: 'orange-500' }].map((x, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 min-h-[300px]">
          <h3 className="text-lg font-semibold text-zinc-900 dark:text-white mb-4 flex items-center gap-2"><BarChart2 size={20} className="text-[#1100ab]" /> Atividade Semanal</h3>
        ...
        <Card className="bg-gradient-to-br from-[#1100ab] to-[#4d4dff] dark:from-[#1100ab]/40 dark:to-[#0a0a0a] border-[#1100ab]/30 flex flex-col justify-between relative overflow-hidden group">
            ...
            <div>
              <h3 className="text-xl font-bold text-white mb-2 flex items-center gap-2">Trabalho Profundo <InfinityIcon size={18} className="text-white dark:text-[#4d4dff]" /></h3>
              <p className="text-zinc-100 dark:text-zinc-300 mb-6 italic text-sm border-l-2 border-white/50 dark:border-[#1100ab]/50 pl-3">"A superficialidade não constrói impérios."</p>
            </div>
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Configurar Meta">
        <form onSubmit={e => { e.preventDefault(); setCountdown({ date: form.date, title: form.title || "Minha Meta" }); setModal(false); }} className="space-y-4">
          <div>
            ...
            <input type="text" placeholder="Ex: ENEM..." className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-[#1100ab]" value={form.title} onChange={e => setForm({ ...form, title: e.target.value })} />
          </div>
          <div>
            <label className="text-xs text-zinc-500 font-bold uppercase">Data Alvo</label>
            <input type="date" required className="w-full mt-1 bg-zinc-100 dark:bg-black border border-zinc-200 dark:border-zinc-700 rounded-2xl p-3 text-zinc-900 dark:text-white outline-none focus:border-[#1100ab]" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} />
          </div>
